...
class GLSLRenderer(CStyleLanguage):
  ...
  string_rewrite = PatternMatcher([
    ...
    (UPat(Ops.INDEX, src=(UPat.var("buf"), UPat.var('idx')), allow_any_len=True),
      lambda ctx,buf,idx: f"{ctx[buf]}[{strip_parens(ctx[idx])}]"),
  ...
  def render_kernel(self, function_name:str, kernel:list[str], args:list[tuple[str, tuple[DType, bool]]], uops:list[UOp], prefix=None) -> str:
    ...
    first_var_pos = next((i for i, (_, (dt, _)) in enumerate(args) if dt == dtypes.int32), len(args))

    meta = {"bufs": first_var_pos, "vars": len(args) - first_var_pos}
    ...
    for i, (buf_name, (buf_dtype, _)) in enumerate(args[:first_var_pos]):
      assert isinstance(buf_dtype, PtrDType)
      k += [f"layout(set = 0, binding = {i}, std430, scalar) buffer buf_{buf_name} {{ {self.render_dtype(buf_dtype)} {buf_name}; }};"]
    if len(args) > first_var_pos:
      k += [f"layout(set = 0, binding = {first_var_pos}, std430, scalar) uniform vals {{ "]
      for i, (var_name, (var_dtype, _)) in enumerate(args[first_var_pos:]):
        assert var_dtype == dtypes.int32
        k += [f"  {self.render_dtype(var_dtype)} {var_name};"]
      k += ["};"]
    k += [""]
    if (shared:=[l[2:] for l in kernel if l.startswith("  shared ")]):
      k += [*shared, ""]
    k += [f"// META: {json.dumps(meta)}"]
  ...
  def render_dtype(self, dt:DType, mutable=True, bool_hack:bool=False) -> str:
    if isinstance(dt, PtrDType):
      return self.render_dtype(dt.base, mutable=mutable, bool_hack=True) + "[]"
    if dt.count > 1:
