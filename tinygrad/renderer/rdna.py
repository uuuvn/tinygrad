...
from tinygrad.renderer.support.rdna_asm import RDNAOp, RDNAOps, RDNAImm, RDNAReg, RDNAValue, SRC9_full, RDNAOPS_VALU, RDNAOPS_LOAD, EXEC_LO, LBL, \
                                               serialize_tinyasm, basewalk, same_reg, DUAL_MAP, DUAL_OP, RDNARegSet
...
class RDNACodegen:
  ...
  def hazards(self):
    vmqueue: list[RDNAReg] = []
    vmqueue_set: set[RDNAReg] = set()
    lgkmqueue: list[RDNAReg] = []
    lgkmqueue_set: set[RDNAReg] = set()
    i = 0
    while i < len(self.rops):
      rop = self.rops[i]
      for arg in rop.args:
        if not isinstance(arg, RDNAReg): continue
        arg, _ = basewalk(arg)
        if arg in vmqueue_set or arg in lgkmqueue_set:
          self.rops.insert(i, RDNAOp(RDNAOps.S_WAITCNT, RDNAImm(dtypes.uint16, 0)))
          vmqueue_set.clear()
          vmqueue = []
          lgkmqueue_set.clear()
          lgkmqueue = []
          i += 1
      if rop.op is RDNAOps.S_BARRIER:
          self.rops.insert(i, RDNAOp(RDNAOps.S_WAITCNT, RDNAImm(dtypes.uint16, 0)))
          vmqueue_set.clear()
          vmqueue = []
          lgkmqueue_set.clear()
  ...
  def fuse(self):
    ...
    for i,rop in enumerate(self.rops):
      ...
      if rop.op in DUAL_MAP and DUAL_MAP[rop.op] < 16:
