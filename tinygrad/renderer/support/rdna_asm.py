...
from tinygrad.helpers import i2u, flatten, all_same, unwrap, getbits
...
class RDNAImm:
  ...
  def from_text(text):
    ...
    return RDNAImm(dtype, float(val) if dtypes.is_float(dtype) else (int(val) if dtype != dtypes.bool else val=='True'))
...
class SOP1(OpEncoding):
  ...
  @staticmethod
  def _encode(opcode:int, sdst_t:tuple[RDNAReg, xREG], src0_t:tuple[RDNAValue, SSRC8]):
...
class DUAL(OpEncoding):
  ...
  def encode(self, *args) -> bytes|None:
    ...
    if len(x_args)>2:
      assert len(x_args) == 3 and isinstance(x_args[2], RDNAReg)
      e_srcx1 = xREG('v', x_args[2].size, ArgInfo(True, False, None)).encode(x_args[2])
    else:
      e_srcx1 = 0
    if len(y_args)>2:
      assert len(y_args) == 3 and isinstance(y_args[2], RDNAReg)
...
class RDNAOp:
  ...
  def from_text(text: str) -> RDNAOp:
    ...
    if op is RDNAOps.S_CBRANCH_SCC1:
      assert len(args) == 1
...
.amdhsa_kernel {name}
  ...
  .amdhsa_kernarg_size {kernarg_len}
...
amdhsa.kernels:
  - .name: {name}
    ...
    .kernarg_segment_size: {kernarg_len}
    .kernarg_segment_align: 8
    .group_segment_fixed_size: {lds_size}
    .private_segment_fixed_size: 0
    .sgpr_count: {(last_reg['v']+3)//4}
    .vgpr_count: {(last_reg['s']+3)//4}
    .args:
      {(chr(10) + '      ').join(yaml.dump(args).splitlines())}
.end_amdgpu_metadata
'''
