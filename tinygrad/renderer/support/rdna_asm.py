...
import functools, struct, subprocess, pickle
from collections import defaultdict
from dataclasses import dataclass
from enum import Enum
from tinygrad.device import Compiler, CompileError
from tinygrad.runtime.support.compiler_hip import compile_hip
from tinygrad.helpers import i2u, unwrap, getbits
...
class VMEM(OpEncoding):
  ...
  def _encode(opcode:int, seg:int, saddr_t:tuple[RDNAReg, xREG]|None=None, vaddr_t:tuple[RDNAReg, xREG]|None=None,
              vdst_t:tuple[RDNAReg, xREG_opsel]|None=None, vdata_t:tuple[RDNAReg, xREG_opsel]|None=None):
    offset = 0
    ...
    return struct.pack('<I', 0b110111 << 26 | opcode << 18 | seg << 16 | offset) + \
...
def tinyasm_to_tinyprogram(tasm: str) -> TinyProgram:
  name = None
  arch = None
  # --
  args: list[dict] = []
  ...
  for line in tasm.splitlines():
    ...
    if components[0].startswith('.'):
      ...
      (rops_trap if in_trap else rops).append(rop)
  ...
  return TinyProgram(
    kernel=b''.join([rop.encode() for rop in rops])+RDNAOp(RDNAOps.S_CODE_END).encode()*64,
    trap_handler=b''.join([rop.encode() for rop in rops_trap])+(RDNAOp(RDNAOps.S_CODE_END).encode()*64 if len(rops_trap) > 0 else b''),
    group_size=lds_size,
    private_size=0,
    kernarg_size=kernarg_len,
    vgpr_count=(last_reg['v']+3)//4,
    sgpr_count=(last_reg['s']+3)//4,
  )

def llvm_dis(name: str, lib: bytes):
  prg = f'''\
...
.end_amdhsa_kernel
'''
  asm = subprocess.check_output(["/opt/rocm/llvm/bin/llvm-objdump", '-d', '-'], input=compile_hip(prg, asm=True))
...
class RDNACompiler(Compiler):
  ...
  def compile(self, src:str) -> bytes:
    try: return pickle.dumps(tinyasm_to_tinyprogram(src))
    except RuntimeError as e: raise CompileError(e) from e
  def disassemble(self, lib:bytes):
    tp: TinyProgram = pickle.loads(lib)
    llvm_dis('kernel', tp.kernel)
    llvm_dis('trap_handler', tp.trap_handler)
