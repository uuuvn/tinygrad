...
from tinygrad.helpers import getenv, to_mv, round_up, data64_le, mv_address, DEBUG, OSX, all_same, flatten
...
class AMDComputeQueue(HWQueue):
  def __init__(self, dev:AMDDevice):
    self.dev, self.gc, self.nbio = dev, dev.gc, dev.nbio
    super().__init__()
  ...
  def pred_exec(self, xcc_mask:int):
    if self.dev.xccs > 1:
      self.pkt3(self.gc.PACKET3_PRED_EXEC, xcc_mask << 24)
  ...
  def acquire_mem(self, addr=0x0, sz=(1 << 64)-1, gli=1, glm=1, glk=1, glv=1, gl1=1, gl2=1):
    if self.dev.gfxver >= 10:
      ...
      cp_coher_cntl = self.gc.PACKET3_ACQUIRE_MEM_CP_COHER_CNTL_SH_ICACHE_ACTION_ENA(1) | \
                      self.gc.PACKET3_ACQUIRE_MEM_CP_COHER_CNTL_SH_KCACHE_ACTION_ENA(1) | \
                      self.gc.PACKET3_ACQUIRE_MEM_CP_COHER_CNTL_TC_ACTION_ENA(1) | \
                      self.gc.PACKET3_ACQUIRE_MEM_CP_COHER_CNTL_TCL1_ACTION_ENA(1) | \
                      self.gc.PACKET3_ACQUIRE_MEM_CP_COHER_CNTL_TC_WB_ACTION_ENA(1)
      self.pkt3(self.gc.PACKET3_ACQUIRE_MEM, cp_coher_cntl, *data64_le(sz), *data64_le(addr), 0x0000000A)
  ...
  def release_mem(self, address=0x0, value=0, data_sel=0, int_sel=2, ctxid=0, cache_flush=False):
    if self.dev.gfxver >= 10:
      ...
      memsel_dw = self.gc.PACKET3_RELEASE_MEM_DATA_SEL(data_sel) | self.gc.PACKET3_RELEASE_MEM_INT_SEL(int_sel) | \
                  self.gc.PACKET3_RELEASE_MEM_DST_SEL(0)

      self.pkt3(self.gc.PACKET3_RELEASE_MEM, event_dw | cache_flags_dw, memsel_dw, *data64_le(address), *data64_le(value), ctxid)
    else:
      op = self.gc.EVENT_TYPE(self.gc.CACHE_FLUSH_AND_INV_TS_EVENT) | self.gc.EVENT_INDEX(self.gc.event_index__mec_release_mem__end_of_pipe) | \
           self.gc.EOP_TC_WB_ACTION_EN | self.gc.EOP_TC_NC_ACTION_EN
      sel = self.gc.DATA_SEL(data_sel) | self.gc.INT_SEL(int_sel)

      self.pkt3(self.gc.PACKET3_RELEASE_MEM, op, sel, *data64_le(address), *data64_le(value), 0)
  ...
  def xcc_barrier(self):
    ...
    mem_eq = self.gc.WAIT_REG_MEM_FUNCTION(WAIT_REG_MEM_FUNCTION_EQ) | self.gc.WAIT_REG_MEM_MEM_SPACE(1)
    self.pkt3(self.gc.PACKET3_ATOMIC_MEM, self.gc.TC_OP_ATOMIC_ADD_RTN_32, *data64_le(a.value_addr), *data64_le(1), *data64_le(0), 10) # a += 1
    self.pkt3(self.gc.PACKET3_WAIT_REG_MEM, mem_eq, *data64_le(a.value_addr), 0, 0b111, 10) # a == 0 (mod 8) via bitmask
    self.pkt3(self.gc.PACKET3_ATOMIC_MEM, self.gc.TC_OP_ATOMIC_ADD_RTN_32, *data64_le(b.value_addr), *data64_le(1), *data64_le(0), 10) # b += 1
    self.pkt3(self.gc.PACKET3_WAIT_REG_MEM, mem_eq, *data64_le(b.value_addr), 0, 0b111, 10) # b == 0 (mod 8) via bitmask
    return self
  ...
  def xcc_config(self):
    for xcc_id in range(self.dev.xccs):
      with self.pred_exec(xcc_mask=1 << xcc_id): self.wreg(self.gc.regCOMPUTE_CURRENT_LOGIC_XCC_ID, xcc_id)
  ...
  def exec(self, prg:AMDProgram, args_state:CLikeArgsState, global_size:tuple[sint, ...], local_size:tuple[sint, ...]):
    ...
    if prg.dev.target >= 100000: self.wreg(self.gc.regCOMPUTE_STATIC_THREAD_MGMT_SE4, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF)
    ...
    if self.dev.xccs > 1: self.release_mem()
    self.xcc_barrier()
    return self
  ...
  def _submit(self, dev:AMDDevice):
    if self.dev.xccs > 1 and dev != self.binded_device: self.bind(dev) # WORKAROUND: PACKET3_PRED_EXEC doesn't work in rings, only in IBs
...
class AMDIP:
  ...
  def module(self):
    for ver in [self.version, self.version[:2]+(0,), self.version[:1]+(0, 0)]:
      try: return importlib.import_module(f"tinygrad.runtime.autogen.am.{self.name}_{'_'.join(map(str, self.version))}")
      except ImportError: pass
    raise ImportError(f"Failed to load autogen module for {self.name.upper()} {'.'.join(map(str, self.version))}")
  @functools.cached_property
...
class AMDDevice(HCQCompiled):
  ...
  def __init__(self, device:str=""):
    ...
    vgpr_size_per_cu = 0x60000 if self.target in {110000, 110001, 120000, 120001} else \
                       0x80000 if (self.target//100)*100 in {90008, 90010, 90400} else 0x40000
    ...
    self.sdma = AMDIP('sdma', self.dev_iface.ip_versions[am.SDMA0_HWIP], self.dev_iface.ip_offsets[am.SDMA0_HWIP])
    ...
    max_copy_size = 0x40000000 if self.sdma.version >= (5, 0, 0) else 0x400000
    ...
    if self.xcc_sync is not None: AMDComputeQueue(self).xcc_config().signal(self.timeline_signal, self.next_timeline()).submit(self)
    ...
    if self.sqtt_enabled:
      ...
      self.cmd_id = 0
      AMDComputeQueue(self).start_trace(self.sqtt_buffers, self.sqtt_itrace_se_mask).signal(self.timeline_signal, self.next_timeline()).submit(self)
