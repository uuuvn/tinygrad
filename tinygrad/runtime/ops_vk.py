...
import subprocess, functools, ctypes, tinygrad.runtime.autogen.vulkan as vk
from typing import Any
from dataclasses import dataclass
from tinygrad.device import LRUAllocator, BufferSpec, Compiler, Compiled
from tinygrad.renderer.llvmir import VKLLVMRenderer
...
class VKLLVMCompiler(Compiler):
  def __init__(self, cachekey:str|None=None):
    super().__init__("compile_llvm_spirv" if cachekey is None else cachekey)
...
!5 = !{!"Simple C++ TBAA"}
'''
    return subprocess.check_output("llc -mtriple=spirv64v1.6-unknown-vulkan1.3-compute -filetype=obj - -o -".split(), input=src.encode("utf-8"))
  def disassemble(self, lib:bytes):
    print(subprocess.check_output(["spirv-dis", "-"], input=lib).decode())
    subprocess.run(["spirv-val", "--target-env", "vulkan1.3", "-"], input=lib).check_returncode()
...
class VKAllocator(LRUAllocator['VKDevice']):
  ...
  def _alloc(self, size:int, options:BufferSpec):
    ...
    mem_props = vk.VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | \
...
class VKDevice(Compiled):
  ...
  def __init__(self, device:str):
    ...
    _ext({"VK_EXT_shader_object": 1}, mandatory=True)
    ...
    super().__init__(device, VKAllocator(self), [(VKLLVMRenderer, VKLLVMCompiler)], functools.partial(VKProgram, self))
...
if __name__ == '__main__':
  ...
  a = Tensor.full((4096,), random.random(), device='VK').contiguous().realize()
  print(a.numpy())
