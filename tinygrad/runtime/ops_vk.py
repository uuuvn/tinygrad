...
import subprocess, shutil, time, functools, ctypes, struct, tinygrad.runtime.autogen.vulkan as vk
from typing import Any
from dataclasses import dataclass
from weakref import WeakSet
from tinygrad.device import LRUAllocator, BufferSpec, Compiler, Compiled
from tinygrad.renderer.cstyle import VKCLRenderer
...
class VKCLCompiler(Compiler):
  def __init__(self, cachekey:str|None=None):
    super().__init__("compile_vkcl" if cachekey is None else cachekey)
  def compile(self, src:str) -> bytes:
    bugs = ["-O0", "--cl-opt-disable", "-max-pushconstant-size=0"]
    stderr = None if DEBUG >= 6 else subprocess.DEVNULL
    lib = subprocess.check_output(["clspv", *bugs, "-", "-o", "-"], input=src.encode("utf-8"), stderr=stderr)
    if shutil.which("spirv-val"):
      subprocess.run(["spirv-val", "--target-env", "vulkan1.3", "-"], input=lib, check=True)
    return lib
  def disassemble(self, lib:bytes):
    subprocess.run(["spirv-dis", "-"], input=lib, check=True)
...
class VKProgram:
  ...
  def __init__(self, dev:VKDevice, name:str, lib:bytes):
    ...
    checkz(vk.vkCreateDescriptorSetLayout(self.dev.ldev, ctypes.pointer(descriptor_set_layout_create_info), None, ctypes.byref(self.descriptor_set_layout)))
    ...
    self.indirect_buffer = self.dev.allocator.alloc(4*3)
    self.vals_buffer = self.dev.allocator.alloc(4*VKProgram.MAX_VALS)
    ...
    checkz(vk.vkCreateFence(self.dev.ldev, fence_create_info, None, ctypes.byref(self.fence)))
  ...
  def __del__(self):
    ...
    if hasattr(self, "cbuf"):
      ...
      del self.cbuf
    if hasattr(self, "vals_buffer"):
      self.dev.allocator.free(self.vals_buffer, 4*VKProgram.MAX_VALS)
      del self.vals_buffer
    if hasattr(self, "indirect_buffer"):
      self.dev.allocator.free(self.indirect_buffer, 12)
  ...
  def __call__(self, *bufs:VKBuf, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1), vals:tuple[int, ...]=(), wait=False):
    ...
    assert len(vals) <= VKProgram.MAX_VALS, "too much vals"
    assert len(bufs) <= VKProgram.MAX_BUFFERS - 1, "too much buffers"
    ...
    arg_writes = [
      ...
      ) for i, buf in enumerate(bufs + (self.vals_buffer,))
    ]
    vk.vkUpdateDescriptorSets(self.dev.ldev, len(arg_writes), (vk.VkWriteDescriptorSet * len(arg_writes))(*arg_writes), 0, None)
    to_mv(self.indirect_buffer.cpu, self.indirect_buffer.sz)[:] = struct.pack("<3I", *global_size)
    to_mv(self.vals_buffer.cpu, self.vals_buffer.sz)[:len(vals)*4] = struct.pack(f"<{len(vals)}I", *vals)
...
# VK_ADD_LAYER_PATH="$(nix build --no-link --print-out-paths nixpkgs\#vulkan-validation-layers)/share/vulkan/explicit_layer.d"
# VK_LOADER_LAYERS_ENABLE=VK_LAYER_KHRONOS_validation
# VK_LAYER_ENABLES=VK_VALIDATION_FEATURE_ENABLE_BEST_PRACTICES_EXT:VK_VALIDATION_FEATURE_ENABLE_SYNCHRONIZATION_VALIDATION_EXT:VK_VALIDATION_FEATURE_ENABLE_GPU_ASSISTED_EXT
class VKDevice(Compiled):
  ...
  def __init__(self, device:str):
    if VKDevice.instance is None:
      ...
      checkz(vk.vkCreateInstance(instance_create_info, None, ctypes.byref(VKDevice.instance)))
    ...
    super().__init__(device, VKAllocator(self), [(VKCLRenderer, VKCLCompiler)], functools.partial(VKProgram, self))
