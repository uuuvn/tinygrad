import sys, platform, tempfile, pathlib, subprocess
...
class ClangJITCompiler(Compiler):
  ...
  def compile(self, src:str) -> bytes:
    machine = platform.machine() if sys.platform != 'win32' else 'x86_64'
    # -fno-math-errno is required for __builtin_sqrt to become an instruction instead of a function call
    # x18 is a reserved platform register. It is clobbered on context switch in macos and is used to store TEB pointer in windows on arm, don't use it
    args = ['-march=native', f'--target={machine}-none-unknown-elf', '-O2', '-fPIC', '-ffreestanding', '-fno-math-errno', '-nostdlib']
