...
from tinygrad.helpers import cpu_time_execution, OSX, getenv, cpu_objdump
...
class ClangJITProgram:
  def __init__(self, name:str, lib:bytes):
    ...
    if OSX:
      ...
      GLOBAL_HANDLE.sys_icache_invalidate(ctypes.c_void_p(mem), ctypes.c_size_t(len(lib)))
...
class ClangDevice(Compiled):
  def __init__(self, device:str):
    super().__init__(device, MallocAllocator, ClangRenderer(),
                     *((ClangJITCompiler(), ClangJITProgram) if getenv("FASTCLANG", 1) else (ClangCompiler(), ClangProgram)))
